...
- `book-editorial.json`(원신흥만 존재) — 발견 성공 화면에서 보여주는 teaser/hook/question. **Data4Library 데이터가 아니라 Lib Quest가 직접 작성한 편집 콘텐츠**다. **P1부터 이 파일은 런타임 조회 대상이 아니다** — `scripts/import-editorial-content.ts`가 최초 1회 `BookEditorial` DB 테이블에 시드하는 baseline이자, `/admin/review`의 "데모 데이터 초기화"가 되돌아가는 기준값 역할만 한다. 이용자 화면은 항상 DB(운영자가 검수·공개한 값)를 조회한다. 아래 "book-editorial.json 작성 방식" 참고.
...
| ISBN 연결 방식 | `isbn13` 키로 연결(제목 문자열 매칭 금지 — 동명이서 오매칭 방지). import 시 같은 도서관의 `Book.isbn13`과 매칭해 `BookEditorial.bookId`로 연결한다(`scripts/import-editorial-content.ts`). |
...
| 현재 적용 범위 | 대전 원신흥도서관 36권 전체(baseline → DB import 완료). 나머지 3개 도서관(갈마·가수원·노은) 108권은 baseline 자체가 없다 — 근거 없는 문구를 시간에 쫓겨 만드는 대신 품질을 우선했다. `/admin/review`에서 해당 책을 열면 `(아직 DB에 콘텐츠 없음)`으로 표시되며(운영자가 그 자리에서 입력해 새로 만들 수 있다), 이용자 화면에서는 미공개 상태라 콘텐츠 영역 자체가 노출되지 않는다(표지·제목·저자·XP는 정상 노출). |
